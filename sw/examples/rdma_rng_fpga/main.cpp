...
int main(int argc, char *argv[])  
{
    ...
    //cproc->netDrop(0, 1, 0);
        ...
        if(opId == "rng") {
            // Flag to indicate if the function should be stopped
            std::atomic<bool> stopFunction(false);
            // Mutex to protect access to the stop flag
            std::mutex stopMutex;
            ...
            auto timeoutFunction = [&stopFunction, &stopMutex, &workerThread, &startTime]() {
                // std::cout << "timeoutFunction running." << std::endl;
                while (std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - startTime).count() < 2000) {
                // Wait for 2 seconds
                std::this_thread::sleep_for(std::chrono::milliseconds(100));
                }

                // Lock the mutex before modifying the stop flag
                std::lock_guard<std::mutex> lock(stopMutex);
                stopFunction.store(true);
                std::cout << "timeoutFunction finished." << std::endl;

                // You can also try to join the thread here to wait for it to finish
                // workerThread.join();
            };

            // Function to be executed in a separate thread (replace with your actual function)
            auto benchThread = [&]() {
                bench.runtime(benchmark_thr);
            };

            // Start the worker thread
            workerThread = std::thread(benchThread);

            // Start the timeout thread
            std::thread timeoutThread(timeoutFunction);

            // // Check if the function has finished or the timeout has expired
            // if (workerThread.joinable()) {
            //     // Wait for the worker thread to finish (if not stopped by timeout)

            //     workerThread.join();
            // }

            std::cout << "about to check mutex" << std::endl;
            // Check the stop flag to see if the timeout function stopped the worker thread
            if (stopFunction.load()) {
                std::cout << "Function timed out after 2 seconds." << std::endl;
                iqp->sendAck(1);
                iqp->closeAck();
                ictx.closeSock();
                return 1;
	    ...
	    std::cout << (bench.getAvg()) / (n_reps_lat * (1 + oper)) << " [ns]" << std::endl;
            ...
            if(oper) {
                ...
                std::cout << std::setw(8) << app_bytes << " [bytes], throughput: " 
                << std::setw(8) << app_bytes * 0.9536 / time * n_reps_thr << " MB/s" << std::endl;
